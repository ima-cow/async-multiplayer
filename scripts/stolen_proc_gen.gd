...
@export var min_cave_size := 80  # Minimum size of cave regions to keep
@export var room_radius := 4  # Radius of open spaces around start/end

# Map settings
@export var map_width := 500
@export var map_height := 500
@export var max_generation_attempts := 5  # Maximum attempts to generate a valid map
	...
	for x in range(map_width):
		for y in range(map_height):
			...
			if !_map[pos]:
				...
				for neighbor:Vector2 in [Vector2(1, 0), Vector2(-1, 0), Vector2(0, 1), Vector2(0, -1)]:
